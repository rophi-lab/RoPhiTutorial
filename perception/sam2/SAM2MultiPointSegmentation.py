...
class SAM2MultiPointSegmentation(BasePerception):
    def __init__(self, config):
        super().__init__(config)
        self.name = "SAM2MultiPointSegmentation"
        ...
        self._bbox_pub_channel = config["pub_manager"].get("bbox_channel", "bbox_data")
        ...
        try:
            ...
            for i, pcd in enumerate(self._point_clouds):
                if pcd is not None:
                    ...
                    if len(points) > 0:
                        # Publish center point of point cloud
                        center = np.mean(points, axis=0)
                        out_pt = Point3DData(num_points=1)
                        out_pt.set_time(rgbd_data.get_time())
                        out_pt.set_position(center.reshape(1, 3))

                        if self.percep_pub_que_dict[self._point_pub_channel]:
                            self.percep_pub_que_dict[self._point_pub_channel].put(
                                out_pt
                            )
                            print(f"Published point cloud center {i+1}: {center}")

            # Publish bounding box data
            bbox_data = []
            for i, bbox in enumerate(self._oriented_bboxes):
                if bbox is not None:
                    # Create bbox data: [center_x, center_y, center_z, extent_x, extent_y, extent_z, rotation_matrix_flattened]
                    bbox_vector = np.concatenate(
                        [bbox.center, bbox.extent, bbox.R.flatten()]
                    )
                    bbox_data.append(bbox_vector)

            if len(bbox_data) > 0:
                bbox_array = np.array(bbox_data)
                out_bbox = NamedVecListData(
                    num_vecs=len(bbox_data),
                    vec_dim=bbox_array.shape[1],
                    name="oriented_bboxes",
                )
                out_bbox.set_data(
                    rgbd_data.get_time(),
                    ["oriented_bboxes"] * len(bbox_data),
                    bbox_array,
                )

                if self.percep_pub_que_dict[self._bbox_pub_channel]:
                    self.percep_pub_que_dict[self._bbox_pub_channel].put(out_bbox)
                    print(f"Published {len(bbox_data)} bounding boxes")
