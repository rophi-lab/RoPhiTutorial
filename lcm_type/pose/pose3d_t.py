"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""


from io import BytesIO
import struct

class pose3d_t(object):

    __slots__ = ["timestamp", "position", "velocity", "tip"]

    __typenames__ = ["float", "float", "float", "float"]

    __dimensions__ = [None, [3], [3], [3]]

    def __init__(self):
        self.timestamp = 0.0
        """ LCM Type: float """
        self.position = [ 0.0 for dim0 in range(3) ]
        """ LCM Type: float[3] """
        self.velocity = [ 0.0 for dim0 in range(3) ]
        """ LCM Type: float[3] """
        self.tip = [ 0.0 for dim0 in range(3) ]
        """ LCM Type: float[3] """

    def encode(self):
        buf = BytesIO()
        buf.write(pose3d_t._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">f", self.timestamp))
        buf.write(struct.pack('>3f', *self.position[:3]))
        buf.write(struct.pack('>3f', *self.velocity[:3]))
        buf.write(struct.pack('>3f', *self.tip[:3]))

    @staticmethod
    def decode(data: bytes):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = BytesIO(data)
        if buf.read(8) != pose3d_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return pose3d_t._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = pose3d_t()
        self.timestamp = struct.unpack(">f", buf.read(4))[0]
        self.position = struct.unpack('>3f', buf.read(12))
        self.velocity = struct.unpack('>3f', buf.read(12))
        self.tip = struct.unpack('>3f', buf.read(12))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        if pose3d_t in parents: return 0
        tmphash = (0x5c2ef673ee81ab0f) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff) + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _packed_fingerprint = None

    @staticmethod
    def _get_packed_fingerprint():
        if pose3d_t._packed_fingerprint is None:
            pose3d_t._packed_fingerprint = struct.pack(">Q", pose3d_t._get_hash_recursive([]))
        return pose3d_t._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", pose3d_t._get_packed_fingerprint())[0]

